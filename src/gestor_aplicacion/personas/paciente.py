    ...
    def calcular_precio(self, servicio):
        if isinstance(servicio, Formula):
            ...
            if self.tipo_eps == "Subsidiado":
                ...
                precio_total = (
                    self._habitacion_asignada.categoria.valor / 2) * self._habitacion_asignada.dias
            else:
                precio_total = self._habitacion_asignada.categoria.valor * \
                    self._habitacion_asignada.dias
            precio_total *= (1 + IVA)
            return precio_total
