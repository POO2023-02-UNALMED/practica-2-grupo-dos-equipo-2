...
def imprimir_titulo(frame):
    ...
    # Imprime el titulo
    titulo = tk.Label(frame, text="Asignar Habitación",
...
def asignar_habitacion(hospital, frame):
    ...
    def asignar_habitacion_a_Pacientes(paciente):
                ...
                if respuesta:
                    ...
                    for i, habitacion in enumerate(hospital._habitaciones):
                        if habitacion.numero == int(
                            ...
                            break
                    messagebox.showinfo(
                        "Habitación asignada", "La habitación se ha asignado exitosamente")
                    mostra_asignacion(paciente)

                else:
                    messagebox.showinfo(
                        "Asignación cancelada", "La asignación de habitación ha sido cancelada")
                    # Se importa aca para evitar una referencia circular
                    from src.ui_main.ventana_principal import implementacion_default
        ...
        combo_tipo_categoria.bind(
            "<<ComboboxSelected>>", habilitar_elegir_habitacion)
        combo_tipo_categoria.grid(
        ...
        combo_elegir_habitacion = ttk.Combobox(
            frame1, textvariable=valor_defecto2, state="disabled")
        combo_elegir_habitacion.bind(
            "<<ComboboxSelected>>", habilitar_ingresar_dias)
        combo_elegir_habitacion.grid(
            row=1, column=1, padx=10, pady=10, sticky="w")

        label_entry = tk.Label(
            frame1, text="Ingrese los dias:", bg="white", font=("Helvetica", 10, "bold"))
        ...
        boton_aceptar = tk.Button(
            frame, text="Aceptar", command=confirmar_habitacion)
