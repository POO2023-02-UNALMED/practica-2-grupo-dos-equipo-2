...
def imprimir_titulo(frame):
    ...
    # Imprime el titulo
    titulo = tk.Label(frame, text="Formular Medicamentos",
...
def formula_medica(hospital, frame):
    def doctor_formula(paciente):
        def habilitar_doctor(event):
            ...
            # Acceder al nombre del objeto seleccionado
            valor_defecto1 = objeto_seleccionado.nombre + objeto_seleccionado.tipologia
            eleccion = combo_enfermedades.get()
            lista_doc_enf = paciente.historia_clinica.buscar_cita(
                objeto_seleccionado.especialidad, hospital)
        ...
        info_paciente = tk.Label(
            frame, text=f"{paciente.nombre} - CC: {paciente.cedula}", bg="white", font=("Helvetica", 10, "bold"))
        info_paciente.pack()
        ...
        enfermedad_tratar = tk.Label(
            frame_datos, text="Seleccione la enfermedad a tratar: ", bg="white", font=("Helvetica", 10, "bold"))
        ...
        elegir_doctor = tk.Label(
            frame_datos, text="Seleccione el doctor que hará la formula:", bg="white", font=("Helvetica", 10, "bold"))
        elegir_doctor.grid(row=1, column=0, padx=10, pady=10, sticky="w")
        valor_defecto2 = tk.StringVar
        combo_elegir_doctor = ttk.Combobox(
            frame_datos, textvariable=valor_defecto2, state="disabled", width=30)
        ...
        def seleccion_medicamentos(enfermedad, formula, doctor):
            if doctor != "":
                ...
                if len(medicamentos_disponibles) != 0:
                    ...
                    titulo_seleccion = tk.Label(
                        frame_seleccion, text="Seleccione los medicamentos", bg="white", font=("Helvetica", 10, "bold"))
                    titulo_seleccion.pack()

                    # Listbox para la selección de medicamentos
                    listbox_medicamentos = tk.Listbox(
                        frame_seleccion, selectmode=tk.MULTIPLE, bg="white")
        ...
        def obtener_seleccion(seleccion, disponibles, formula, doctor):
            ...
            medicamentos_seleccionados = [disponibles[indice]
            ...
            if len(medicamentos_seleccionados) != 0:
                ...
                label_meds_seleccionados = tk.Label(
                    frame_meds_seleccionados, text="Estos son sus medicamentos", bg="white", font=("Helvetica", 10, "bold"))
                ...
                for med in medicamentos_seleccionados:
                    ...
                    label_med = tk.Label(
                        frame_meds_seleccionados, text=string_info, bg="white")
                    label_med.pack()
            ...
            def generar_formula(formula_paciente, doctor):
                respuesta = tk.messagebox.askyesno(
                    "Confirmar Formula", "¿Estas seguro de generar esta formula?")
                if respuesta:
                    ...
                    for doc in hospital.lista_doctores:
                        if doc.nombre == doctor.split(" ")[0]:
                            ...
                            paciente.historia_clinica.lista_formulas.append(
                                formula_paciente)
                            messagebox.showinfo(
                                "Formula Generada", "La formula se ha generado exitosamente")
                            mostrar_historial(paciente)
                else:
                    messagebox.showinfo("Formula cancelada",
                                        "La formula ha sido cancelada")
                    # Se importa aca para evitar una referencia circular
                    from src.ui_main.ventana_principal import implementacion_default
                    implementacion_default(frame)
    ...
    def mostrar_historial(paciente):
        ...
        label = tk.Label(frame_formulas, text="Historial de formulas",
                         bg="white", font=("Helvetica", 10, "bold"))
        label.pack(fill="both")
        for formula in paciente.historia_clinica.lista_formulas:
            formula_label = tk.Label(
                frame_formulas, text=formula.descripcion_servicio())
            formula_label.pack(fill="both", expand=True)
        # Se importa aca para evitar una referencia circular
        from src.ui_main.ventana_principal import implementacion_default

        boton_regresar = tk.Button(
            frame_formulas, text="Regresar", command=lambda: implementacion_default(frame))
        boton_regresar.pack()
    ...
    label_ingreso_cedula = tk.Label(
        frame, text="Ingrese la cédula del paciente:", bg="white", font=("Helvetica", 10, "bold"))
    ...
    boton_buscar_paciente = tk.Button(
        frame, text="Buscar", command=buscar_paciente)
