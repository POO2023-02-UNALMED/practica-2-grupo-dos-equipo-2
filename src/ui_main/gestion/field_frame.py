...
class FieldFrame(Frame):
    ...
    def __init__(self, frame, tituloCriterios, criterios, tituloValores, valores, habilitado, ancho_entry=20):
        ...
        Label(self, text=tituloCriterios, bg="bisque", font=(
            "Comic Sans MS", 15, "bold")).grid(row=0, column=0)
        Label(self, text=tituloValores, bg="bisque", font=(
            "Comic Sans MS", 15, "bold")).grid(row=0, column=1)

        # Etiquetas y campos de entrada para cada criterio
        for i, criterio in enumerate(criterios, start=1):
            Label(self, text=criterio, bg="bisque", font=("Comic Sans MS", 15, "bold")).grid(
