...
def agendar_citas(hospital, frame):
    ...
    def mostrar_historial_citas(paciente):
        ...
        historial_citas_text = tk.Text(
            frame, bg="white", font=("Helvetica", 14))
        ...
        historial_citas_text.config(
            highlightthickness=5, highlightbackground="#4D5BE4")
        ...
        boton_regresar = tk.Button(
            frame, text="Regresar", command=lambda: implementacion_default(frame))
        boton_regresar.pack()
    ...
    def agendamiento_de_la_cita(paciente):
        ...
        def confirmar_cita():
            ...
            if eleccion:
                respuesta = tk.messagebox.askyesno(
                    "Confirmar cita", "¿Estas seguro de agendar esta cita?")
                if respuesta:
                    for doctor in paciente.buscar_doctor_por_eps(combo_tipo_cita.get(), hospital):
                        if doctor.nombre == combo_elegir_doctor.get():
                            cita_agendada = doctor.actualizar_agenda(
                                paciente, combo_elegir_cita.current()+1, doctor.mostrar_agenda_disponible())
                            paciente.actualizar_historial_citas(cita_agendada)
                    messagebox.showinfo(
                        "Cita agendada", "La cita se ha agendado exitosamente")
                    mostrar_historial_citas(paciente)
                else:
                    messagebox.showinfo(
                        "Cita cancelada", "La cita ha sido cancelada")
                    # Se importa aca para evitar una referencia circular
                    from src.ui_main.ventana_principal import implementacion_default
        ...
        info_paciente = tk.Label(
            frame, text=f"{paciente.nombre} - CC: {paciente.cedula}", bg="white", font=("Helvetica", 12))
        ...
        tipo_cita = tk.Label(frame1, text="Seleccione el tipo de cita:",
                             bg="white", font=("Helvetica", 10, "bold"))
        tipo_cita.grid(row=0, column=0, padx=10, pady=10, sticky="w")
        valor_defecto1 = tk.StringVar
        combo_tipo_cita = ttk.Combobox(frame1, values=[
                                       "General", "Odontologia", "Oftalmologia"], textvariable=valor_defecto1, state="readonly")
        ...
        elegir_doctor = tk.Label(
            frame1, text="Seleccione el doctor de su preferencia:", bg="white", font=("Helvetica", 10, "bold"))
        elegir_doctor.grid(row=1, column=0, padx=10, pady=10, sticky="w")
        valor_defecto2 = tk.StringVar
        combo_elegir_doctor = ttk.Combobox(
            frame1, textvariable=valor_defecto2, state="disabled")
        ...
        elegir_cita = tk.Label(frame1, text="Seleccione una fecha para su cita:",
                               bg="white", font=("Helvetica", 10, "bold"))
        elegir_cita.grid(row=2, column=0, padx=10, pady=10, sticky="w")
        valor_defecto3 = tk.StringVar
        combo_elegir_cita = ttk.Combobox(
            frame1, textvariable=valor_defecto3, state="disabled")
        ...
        boton_aceptar = tk.Button(
            frame, text="Aceptar", command=confirmar_cita)
    ...
    titulo_ingreso_cedula = tk.Label(
