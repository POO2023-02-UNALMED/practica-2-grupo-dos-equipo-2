...
def facturacion(hospital, frame):
    ...
    def confirmar_pago(paciente, servicios):
        respuesta = tk.messagebox.askyesno(
            "Confirmar pago", "¿Desea realizar el pago?")
        if respuesta:
            for servicio in servicios:
                servicio.confirmar_pago(paciente, servicio.id_servicio)
            messagebox.showinfo(
                "Pago realizado", "El pago se ha realizado exitosamente")
        else:
    ...
    def calcular_precio(paciente, servicios):
        ...
        for servicio in servicios:
            ...
            label_servicio.bind(
                "<Leave>", lambda event: event.widget.config(highlightthickness=0))

            # Seleccionado por defecto

        label_precio_final = tk.Label(
            frame, text=f"Precio total: ${precio_final:,}", bg="white")
    ...
    def obtener_servicios_sin_pagar(paciente):
        ...
        lista_servicios_sin_pagar = Servicio.obtener_servicios_sin_pagar(
            paciente)

        if len(lista_servicios_sin_pagar) == 0:
            tk.messagebox.showinfo(
                "Sin servicios pendientes", "Usted no tiene ningún servicio pendiente de pago")
            implementacion_default(frame)
            return
        ...
        for servicio in lista_servicios_sin_pagar:
            label_servicio = tk.Label(
                frame, text=servicio.descripcion_servicio(), bg="white")
            label_servicio.pack()
